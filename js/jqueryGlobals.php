...
if ( class_exists( 'ms_Main' ) ) {
	...
	$cleanPreferentialLeaguesDataLive = str_replace(' ', "', '", ms_Main::$settings['ms_preferential_leagues']);
} else {
    $ms_api_key =  $_COOKIE['ms_api_key'];
	$cleanPreferentialLeaguesDataLive = str_replace(' ', "', '", $_COOKIE['ms_preferential_leagues']);
}
        ...
        var fi = setInterval(function() {
            if (jQuery('#widgetLiveScore-js').length > 0) {
                jQuery(widgetLiveScoreLocation).unbind().removeData().html('').widgetLiveScore({
                    ...
                    'method': 'Livescore',
                    'timezone': getTimeZone(),
					'preferentialLeagues': ['<?php echo $cleanPreferentialLeaguesDataLive;?>']
                });
                clearInterval(fi);
            }
        }, 10);
